...
#####问题解答：
为了解答这些问题，我特意重新在新目录下使用了克隆，一下实验证据也均是我自己亲自测试过的。
1. 正确；
...
可见，确实所有的分支均被克隆，且只有HEAD指向的分支(remotes/origin/master)被检出。
2. 正确;</br>git log,git status,git checkout,git commit均不会访问远程版本库。
3. 错误；</br>解释及实验证据同回答1。
4. 正确；</br>克隆后工作区的默认分支处于master分支。实验证据同回答1。
